# Xenium_Prime_OC_analysis_script.R
...
dirname <- "Xenium_Prime_OC"
samplename <- "Xenium_Prime_OC"

spe <- readXeniumSPE(dirName=dirname, sampleName=samplename, 
                     type="HDF5", addFOVs=FALSE)

spe <- readAndAddPolygonsToSPE(spe, boundariesType="parquet")

# In Xenium Prime there is a new class of intergenic control probes that was not
# included in the default list.

spe <- spatialPerCellQC(spe, negProbList=c("NegControlProbe", 
                                           "NegControlCodeword", 
                                           "UnassignedCodeword", 
                                           "DeprecatedCodeword", "Intergenic"), 
                        rmZeros=FALSE)
